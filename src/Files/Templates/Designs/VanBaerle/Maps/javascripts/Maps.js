    ...
    var settings = mergeRecursive(mergeRecursive({}, Dynamicweb.MapSettings), options),
      ...
      // @see http://www.movable-type.co.uk/scripts/latlong.html
        ...
        init = function () {
          ...
          if ( element ) {
            listPosition = $(element).attr('data-list-position');
            if ( listPosition ) {
          ...
          if ( obj_loginLocation.str_country != "" ) {
            $("select[name=country]").val(obj_loginLocation.str_country).change();
            $("select[name=locality]").val($("[data-citycode="+obj_loginLocation.str_state+"]").attr("data-city")).change();
          }
        };
...
(function () {
  ...
  var options = {elementId:'Maps-85', map:{elementId:'Maps-85-map', center:[ 41.771312, -101.601563 ], zoom:5, mapType:'ROADMAP'}, marker:{icon:'/Files/Templates/Designs/VanBaerle/images/marker.png', showInfoWindowAction:'click'}, infoWindow:{}, list:{elementId:'Maps-85-list', showInfoWindowAction:'click', showInfoWindowZoom:0}, clusterer:{styles:[], gridSize:0, enabled:false}};
  new Dynamicweb.Map(options);
}());
