...
function onLoadCartStep1(obj_params) {
    ...
    var bol_firstTime = true,
        ...
        fcn_address2Change = function ($obj_this) {
            ...
            for (var k in json_addresses[$obj_this.val()]) {
                ...
                if (k == "EcomOrderDeliveryRegion" && json_addresses[$obj_this.val()][k] == "") {
        ...
        fillShippingAddress = function(obj){
        ...
        //run
        $("#selectAddress2").selectpicker();
        setTimeout(function(){
            $("#selectAddress2").trigger("change");
        }, 300);
       
        // $("#selectAddress2").selectpicker("refresh");
        // console.log("data", obj_params);


        $("#selectAddress2").on("change", function(){
            $("#shippingInformation input").removeClass("error");
            var value = $(this).val();
            if(value == "newAddress") {
                $("#shippingInformation .editable input").prop("readonly", false);
                $("#shippingInformation .editable input").val("");

            } else {
                var addressObject = JSON.parse(obj_params.addresses)[value];
                fillShippingAddress(addressObject);
            }
    ...
    var lang = $("body").attr("data-language") == "en" ? "" : $("body").attr("data-language");
    ...
    $obj_datepicker.datepicker({
        ...
        onClose: function (selectedDate) {
            if (selectedDate != "") {
                // selectedDate = selectedDate.replace(/\-0/g, "-");
                $("#RequestedDeliveryDate").val(selectedDate);
                //console.log(selectedDate);
                //$("#EcomOrderRecurringEndDate_year").val(selectedDate.split("-")[2]);
                //$("#EcomOrderRecurringEndDate_day").val(selectedDate.split("-")[1]);
                //$("#EcomOrderRecurringEndDate_month").val(parseInt(selectedDate.split("-")[0]));
            }
        }
    });
    ...
    function validateFields(arr){
        var errors = _.reduce(arr, function(result, value, key){
    ...
    $('#shipping-step').on("submit", function(e){
        // alert("INTRASUBMIT");
        var validateFieldsArray = ["#EcomOrderDeliveryCountry", "#EcomOrderDeliveryRegion","#EcomOrderDeliveryCity","#EcomOrderDeliveryZip", "#EcomOrderDeliveryAddress", "#EcomOrderDeliveryCompany"];
        
        // e.preventDefault();
        if(validateFields(validateFieldsArray) > 0) {
            e.preventDefault();
...
function onLoadCartStep2(str_alertErrors, webSeviceConnectionStatus) {
    ...
    $('input[name=EcomCartShippingmethodID], input[name=EcomCartPaymethodID]').change(function () {
        if ($(this).is(':checked')) {
            ...
            loading(true);
            requestAjax({
                str_url:$obj_form.attr('action'),
                obj_data:$obj_form.serialize(),
                fnc_callback:reloadCartStep
            });
        }
    ...
    $obj_form.submit(function () {
        ...
        if ($obj_termsAndConditions.length && !$obj_termsAndConditions.is(':checked')) {
...
function onLoadCartLogin() {
    ...
    $("#returningLogin > div:first form").addClass("col-sm-6")
        .after($("#dontHaveAnAccount"));
...
function addressValidator() {
    ...
    $("#changeAll").change(function () {
        if ($(this).prop("checked")) {
            $(this).closest(".modal-content").find("input").not("#changeAll").prop("checked", true);
        }
        else {
            $(this).closest(".modal-content").find("input").not("#changeAll").prop("checked", false);
        }
    });
    if ($(".addressChange").length > 0) {
...
function minicart() {
    ...
    var obj_myCartLi = $("#topMenu > li.myCart");

    if (obj_myCartLi.length > 0) {
        requestAjax({
            ...
            fnc_callback:function (obj_response) {
                ...
                if (num_quantity > 0) {
                    $("#miniCart").remove();
                    if (obj_myCartLi.find(".cart-badge__number").length) {
                        obj_myCartLi.find(".cart-badge__number").html(num_quantity);
                    }
                    else {
                ...
                $(".miniCartBtns  > a:first").attr("href", $("li.myCart > a").attr("href"));
                    ...
                    if(pageID == "174" ) {
                        $('.minicart-cart').hide();
                        $('.minicart-continue, .minicart-checkout').show();
                        $('.minicart-list').hide();
                        console.log("intra1");
                    } else if (pageID == "175") {
                        $(".minicart-cart, .minicart-continue, .minicart-checkout").hide();
                        console.log("intra2");
                        $('.minicart-list').show();
                    } else {
                        console.log("intra3");
                        $('.minicart-cart, .minicart-checkout').show();
                        $('.minicart-continue').hide();
                        $('.minicart-list').show();
                    }
...
function reloadCartStep(str_source) {
    $('#cartRightContent').html($(str_source).find('#cartRightContent').html());
    loading(false);
}
